...
async def on_chat_start() -> None:
    ...
    # Per-Chainlit-session thread. Each browser session gets its own
    # LangGraph thread so distinct conversations stay distinct in the
    # checkpointer. The previous "stable per-owner" thread collapsed
    # every browser session into one chainlit-<owner> thread, which
    # ballooned with mixed-topic state and ran up cumulative token
    # cost. A page reload starts a fresh thread; proper "resume this
    # past session" requires Chainlit's data layer (separate work).
